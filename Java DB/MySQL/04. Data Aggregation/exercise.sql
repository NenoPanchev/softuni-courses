...
SELECT w1.`first_name` AS `host_wizard`,
	...
	(SELECT w2.`first_name`
	FROM wizzard_deposits AS w2
	WHERE w2.`id` = w1.`id` + 1) AS `guest_wizard`,
	(SELECT w2.`deposit_amount`
	FROM wizzard_deposits AS w2
	WHERE w2.`id` = w1.`id` + 1) AS guest_wizard_deposit,
	(SELECT SUM(w1.`deposit_amount` - w2.`deposit_amount`)
    FROM wizzard_deposits AS w1,
    wizzard_deposits AS w2) AS difference
FROM wizzard_deposits AS w1
GROUP BY `host_wizard`;

SELECT SUM(`id` - `age`)
FROM wizzard_deposits
WHERE `id`= 16;

SELECT SUM(w1.`deposit_amount` - w2.`deposit_amount`) AS `sum_difference`
FROM `wizzard_deposits` AS w1,
